...
const shortUrlBackend = process.env.VUE_APP_MYURLS_DEFAULT_BACKEND + '/api/link?url='
...
export default {
  ...
  methods: {
    ...
    makeShortUrl() {
      ...
      this.loading = true;
      let finalUrl = shortUrlBackend + encodeURIComponent(this.customSubUrl); // 使用 encodeURIComponent 进行 URL 编码
      ...
      this.$axios
        ...
        .then(res => {
          if (res.data.code === 200 && res.data.data !== "") {
            this.curtomShortSubUrl = res.data.data.replace(/\\+/g, '');
        ...
        .catch(error => {
          // 提示用户错误信息，可以根据需要选择提示的方式
          this.$message.error("短链接获取异常: ", error);
        })
        .finally(() => {
          this.loading = false;
        });
